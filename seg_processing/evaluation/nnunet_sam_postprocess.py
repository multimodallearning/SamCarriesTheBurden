...
seg_processor = SegEnhance(refiner, 'largest', 'dilation', 'square', 8, 'cpu')

if plot_results:
    dir_name = str.join('_', prompts2use1st)
    if sam_refiner.self_refine:
        dir_name += '_self_refine_' + str.join('_', prompts2use2nd)
    plot_save_path = Path(f'/home/ron/Downloads/SAM_refine_result/nnUNet/{sam_type}/' + dir_name)
...
for img, y, file_name in tqdm(ds, unit='img'):
    ...
    # refinement
    refined_sam_masks, est_dice = seg_processor.enhance(nnunet_mask, file_name)

    dsc_unet.append(multilabel_dice(nnunet_mask.unsqueeze(0), y))
    ...
    if plot_results:
        ...
        sam_prompt_debug_plots(PromptExtractor(seg_processor.last_preprocessed_seg > 0.5), img, nnunet_mask, refined_sam_masks,
...
print(f'nnUNet DSC: {dsc_unet.nanmean()}')
