...
def bin2dec(x: torch.Tensor):
    ...
    num_bits = x.shape[1]
    mask = 2 ** torch.arange(num_bits, dtype=torch.long, device=x.device).view(1, -1, 1, 1)
    x_decimal = x * mask
    return x_decimal.sum(dim=1, keepdim=True).long()
...
ds = DentalDataset('train', number_training_samples=43)

# create folder structure
dataset_name = f'Dataset200_DentalSeg'
...
for i, lbl_comb in enumerate(lbl_tmp.unique().long()):
    if lbl_comb.item() == 0:  # skip background
        continue
    lbl_idx_mapping[lbl_comb.item()] = i
del lbl_tmp

for img, lbl_bin, file_name in tqdm(ds, desc='Processing images', unit='image'):
